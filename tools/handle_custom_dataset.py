...
def record_ann(model_meta, img_id, ann_id, images, annotations):
    ...
    k_info = yaml.load(open(os.path.join(data_root, "info.yml"), 'r'), Loader=yaml.FullLoader)
    gt = yaml.load(open(os.path.join(data_root, 'gt.yml'), 'r'))
    ...
    for ind in tqdm.tqdm(inds):
        ...
        R = np.array(gt[ind][0]['cam_R_m2c']).reshape(3, 3)
        t = np.array(gt[ind][0]['cam_t_m2c']) * 0.001
...
def custom_to_coco(data_root, tless_cls):
