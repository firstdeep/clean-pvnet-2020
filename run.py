...
def run_evaluate():
    # python run.py --type custom tless_cls 9 test.file 18
    # python run.py  --type evaluate --cfg_file configs/custom.yaml test.dataset CustomTest model mytless_09 tless_cls 9 test.file 18
    from lib.datasets import make_data_loader
    ...
    print("Check All : " + str(cfg.test.all))
    print("Test File number : " + str(cfg.test.file))
    print("Number of image: " + str(count))
    evaluator.summarize()


def run_visualize():
    # python run.py --type custom tless_cls 9 test.file 18
    # python run.py --type visualize --cfg_file configs/custom.yaml test.dataset CustomTest model mytless_09 tless_cls 9 test.file 18
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        visualizer.visualize(output, batch, cfg.tless_cls, cfg.test.file)
...
def run_custom():
    from tools import handle_custom_dataset, handle_custom_test
    # for train
    # python run.py --type custom tless_cls 9
    # data_root = 'data/custom'
    # handle_custom_dataset.get_mask(data_root, cfg.tless_cls)
    # handle_custom_dataset.custom_to_coco(data_root, cfg.tless_cls)

    # for test
    # python run.py --type custom tless_cls 9 test.file 18
    data_root = 'data/custom_test'
    handle_custom_test.get_mask(data_root, cfg.test.file)
    handle_custom_test.custom_test_to_coco(data_root, cfg.tless_cls, cfg.test.file)
